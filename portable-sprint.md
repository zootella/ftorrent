...
2. **Lock the download folders.** Built in `folders.rs` as two one-folder commands, `folder_lock` and `folder_unlock`, with the page deciding which folders to lock and when. Startup locks every listed folder that exists and never makes one, so a fresh install shows its default folder as not on this machine. A **Prepare download folder** button on the main page stands in for starting a torrent: it makes the default folder with `disk_mkdir` if it isn't there, then locks it, and starting a torrent will call the same store function. The engine is handed only the folders this copy holds. On the dev build, measured: no folder at startup; the button makes the folder and its lock, held; a folder whose lock another process holds shows as in use by another copy, and preparing it changes nothing until that process lets go; one folder listed under two names is held once. Left:
	- On Windows, `.ftorrent` gets the hidden attribute; it's written and waits to be compiled and checked there, in the current letter.
	...
	- When the engine starts loading torrents, it must get each folder once. Two spellings of one folder, like `./downloads` and its absolute path, are both held today and would both be handed to the engine, so `folder_lock` should answer the folder's real path beside its state, and the page hand the engine each real path once.
