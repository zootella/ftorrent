...
- **The Mac portable bundle.** `tauri build --config tauri.portable.conf.json --bundles app` applies a second file over `tauri.conf.json` as a JSON merge patch, and the result is also the config compiled into the app. The patch sets `identifier` to `com.ftorrent.portable`, and a `null` deletes a key, which is how the portable bundle leaves out the document types and URL schemes the installed one declares. Which keys it nulls follows however the Mac associations work puts those declarations in, which isn't built yet; Windows no longer uses `bundle.fileAssociations` at all, since `associate.js` registers from the installed copy itself. `--bundles app` builds the `.app` and no disk image.
...
2. **Lock the download folders.** Built on two general commands in `locks.rs`, `lock_take` and `lock_release`, which lock any file; the page decides which folders to lock and when, makes each folder's `.ftorrent`, hides it with `disk_hide`, and locks `.ftorrent/ftorrent.lock` inside it. Startup locks every listed folder that exists and never makes one, so a fresh install shows its default folder as not on this machine. A **Prepare download folder** button on the main page stands in for starting a torrent: it makes the default folder with `disk_mkdir` if it isn't there, then locks it, and starting a torrent will call the same store function. The engine is handed only the folders this copy holds. On the dev build, measured: no folder at startup; the button makes the folder and its lock, held; a folder whose lock another process holds shows as in use by another copy, and preparing it changes nothing until that process lets go; one folder listed under two names is held once. Left:
	- On Windows, `.ftorrent` gets the hidden attribute through `disk_hide`; it's written and waits to be compiled and checked there, in the current letter.
	- On an installed Mac copy, whether checking a folder inside Downloads at startup raises the privacy prompt; the dev build runs under the terminal's permissions and can't show it, so it's checked with the build at the end.
	- When the engine starts loading torrents, it must get each folder once. Two spellings of one folder, like `./downloads` and its absolute path, are both held today and would both be handed to the engine, so `lock_take` should answer the lock file's real path beside its state, and the page hand the engine each real folder once.
