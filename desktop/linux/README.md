...
The four-package pipeline ran the next day, 2026-Sep-22, in ten minutes and seven seconds end to end: three images checked against the cache in seconds, the two Tauri runs, the Flatpak wrap, and the three checks. One honest note on that time: the Flatpak image's runtime layers were already in Docker's cache on this Mac, so this run did not pay for the five-gigabyte download a new machine would. The three images come to about 11.4 GB on disk, of which the Flatpak image is 5.5 GB.
