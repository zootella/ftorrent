...
Two practical notes. It runs on every launch, which is cheap because each value is read before it is written and an unchanged value is not touched; the shell is only notified if something actually moved. And it does nothing in a debug build, because the command paths come from the executable's own location, and a debug build's executable lives in target/debug, where it will be replaced and eventually deleted; registering it would leave the user's registry pointing at a moving target. A portable copy registers nothing either, for the same reason with a stick in place of a build folder: ejected, it would leave a dead association pointing at a path that no longer exists.
*/

//what ftorrent can open, and the names those things carry in Explorer's Type column and in the Settings app; read only by the windows release path below, so every other build is right that nothing touches them
...
const FILE_TYPES: [(&str, &str, &str); 2] = [//the extension with its dot, the ProgID, and the name a user reads
	...
	(".ftorrent", "ftorrent.ftorrent", "ftorrent File"),//a placeholder until the format exists; the plumbing is the same
];
#[cfg(all(target_os = "windows", not(debug_assertions)))]
...
pub fn associate_register(app: &AppHandle) {
	let installed = app.state::<Paths>().mode == "installed";//a portable copy registers nothing, and neither does a translocated one
	let line = if installed { register().unwrap_or_else(|trouble| trouble) } else { String::new() };//trouble is reported the same way as a result, as a line on the page
...
fn register() -> Result<String, String> {
	Ok(String::new())//blank, so the page shows nothing: a mac launch that was never going to register anything is not news, and a debug build excuses itself for the reason in the essay
}
