...
**The commands**, in package.json, named so each says where it stops:
...
`compile` never makes an installer, `installer` never hashes, `hash` never builds, and `upload` never builds. A name means the same thing on every machine while doing different work underneath: `installer` makes a `.dmg` on macOS and an `.exe` on Windows, and `hash` and `upload` act on whichever this machine can build. The Linux packages use the same vocabulary in the workspace nested at `linux/`, which has a guide of its own. Everything past calling tauri lives in `scripts.js`, reached by a verb, and its essay says where the published names come from: `ftorrent.dmg` and `ftorrent.exe` bare, `ftorrent.amd64.deb`, `ftorrent.arm64.deb`, `ftorrent.x86_64.rpm`, and `ftorrent.x86_64.flatpak` naming their architecture in each ecosystem's own word, none carrying a version, so a link anyone shares keeps pointing at the current build. `hash` writes a small JSON sidecar beside each package with its size, hash, version, and date, into `release/`; the packages there are gitignored and the sidecars are committed. `upload` sends each package and then its sidecar to the site's root, so they are served as `ftorrent.com/ftorrent.dmg` and `ftorrent.com/ftorrent.dmg.json`, overwritten in place every release. It reads its destination and key from `upload.hide.env` in this workspace, which is gitignored and written on each machine that uploads; the comment above `upload` in `scripts.js` lists the five values.

`icons` reads three SVG sources under `src-tauri/icons/`, which `icon-studio/Update-Icons.ps1` writes from the one drawing kept there, `brand.svg`; the studio, Windows only and by hand, is also where the `.torrent` document icon is drawn and packed, and its own README has the whole flow. The three sources differ by one number each: `app-icon.svg` fills its canvas, for Windows and Linux and everything `tauri icon` generates by default; `app-icon-mac.svg` is drawn smaller, because macOS does not mask icons and expects the inset baked into the file, so a full-bleed icon reads a quarter larger than its neighbors in the Dock; and `app-icon-tile.svg` is smaller again, for the Windows 10 Start menu tile, where the mark sits inset on the tile's background. Each of the two extra runs writes a whole tree into a gitignored scratch folder, `.mac/` or `.tile/`, and `scripts.js icons-collect` copies out the one or two files wanted: `mac/icon.icns`, which `bundle.icon` names in place of the full-bleed one, and the two tile PNGs, which `bundle.resources` lands beside the executable with `ftorrent.VisualElementsManifest.xml`, the file Windows reads from the executable's own folder to draw the tile. The mark itself is drawn on a 16 unit grid, so its edges land on whole pixels at the smallest sizes, and the file says `width="1024"` with `viewBox="0 0 16 16"`, which is how an SVG displays large while its coordinates stay small; the two smaller sources are the same two elements seen through a wider viewBox. Run `icons` after changing the artwork, and also after upgrading the Tauri CLI: the generated files are committed artifacts, frozen at the CLI that made them, and a CLI fix, like the one in 2.9.3 that cleared a grey fringe from every curved edge, reaches them only when someone re-runs the generator. Expect `icon.icns` to show as modified after every run whether or not the artwork changed; the CLI writes its entries in an unstable order, and the image bytes are identical.

Two things about `local`. A pnpm script cannot be named `run`, since pnpm's builtin shadows it. And `local` fails with a stack trace that reads like a bug if a dev server is already running: vite.config.js sets `port: 1420` with `strictPort: true`, because Tauri needs to find the frontend at a fixed address, so the second one exits rather than sliding to another port. `lsof -ti :1420` on macOS, or `netstat -ano | findstr :1420` on Windows, names the process already holding it. Note that `tauri dev` and `tauri build` compile into separate profile directories, target/debug and target/release, which share no artifacts, so the second full compile after the first is expected.

**Which command a change deserves.** While working, the two halves are quicker on their own: `pnpm vite-build` finishes in a fraction of a second and catches every frontend error, and `cargo check` from `src-tauri` is nearly as quick and catches every Rust error, including a mistyped capability or permission identifier, which Tauri validates as it compiles. `pnpm compile` is the gate before handing work over, because it is the cheapest single command that runs the frontend build and a real release compile and link; `cargo check` type-checks without generating or linking any code, so it cannot stand in for that. `pnpm installer` when something has to be installed, and before a release or a handoff to another platform. On macOS its last step mounts the disk image and drives Finder to position the icons and set the window background, so Finder windows open and close on the desktop while it runs; that is the bundler working, not a fault. Two things to expect: the release profile shares nothing with the debug profile `pnpm local` uses, so the first release build after a stretch of dev work compiles everything over again; and a green build says the code compiles, not that it works. Only running the app tells you that.

**tauri.conf.json** carries the identity card, the values every platform's package is stamped with. The product name is `ftorrent`, lowercase everywhere, which is the brand. The identifier is `com.ftorrent.ftorrent`, the domain reversed and the app's name, three components as macOS bundle identifiers and Flatpak expect; the scaffold command above wrote the two-component `com.ftorrent`, and it was changed before any release because the identifier names the app data directory on every platform, the macOS bundle identifier that Launch Services and the privacy prompts key on, and the Windows uninstall entry, and changing it after a release would leave orphans of all three on every user's machine. Beside those: publisher `ftorrent`, the homepage, `Copyright © 2026 ftorrent`, license `GPL-3.0-only`, the one-line description and its longer form, category `Utility`, and a macOS minimum system version of 15.0, because the libtorrent wheel the engine is frozen against is built for macOS 15 and an installer that accepted macOS 14 would deliver an engine that cannot load. Cargo.toml carries the same description and an authors line that becomes the `.deb`'s Maintainer field, the project rather than a person, since it is public in every installed copy. `beforeBuildCommand` points at `pnpm vite-build`. The `windows` list is empty on purpose: the window is made in Rust, from `window.rs`, once startup knows whether this launch is the running copy, at the size and place the settings file remembers.

**Bundle targets.** The scaffold ships `"targets": "all"`, which builds everything each platform can build — on Windows an MSI beside the NSIS installer, on Linux an AppImage beside the Debian package. ftorrent distributes six packages, and the four Tauri targets a Mac, a Windows box, or a Linux checkout use are named instead:

```
"targets": ["app", "dmg", "nsis", "deb"]
```

One list serves all three platforms: a target that doesn't apply to the machine doing the build is skipped, so macOS produces the .app and .dmg, Windows the NSIS .exe, and a Linux checkout the .deb. The skipping is silent — a Windows build simply makes the one installer, with no notice that three of the four names didn't apply. The other two Linux packages never appear in this list: the pipeline in `linux/` asks its own containers for `deb,rpm` as an argument to `tauri build`, and wraps the Flatpak from the x86-64 `.deb` rather than through Tauri at all. Beside the targets, `bundle.linux` points the deb and rpm bundlers at `ftorrent.desktop.hbs`, a desktop-entry template whose one departure from Tauri's default is its categories: `Network;FileTransfer;P2P;` is what qBittorrent and Transmission declare, so launchers and app menus file ftorrent beside them rather than under Utility. The portable zip is assembled separately, later.
...
**The engine.** The client's BitTorrent and WebTorrent work happens in a second process rather than in the app's own: a small Python program that holds [libtorrent](https://www.libtorrent.org/), frozen together with its interpreter and libtorrent into a folder the app carries beside itself. Two documents on [docs.ftorrent.com](https://docs.ftorrent.com/) carry the reasoning: the desktop architecture document explains the processes, the files, and the pipes between them, and the libtorrent provenance document records where libtorrent comes from and the hashes we pin. This section is how the workspace is arranged around them.
...
The freeze is a folder, the executable `ftorrent-engine` beside an `_internal` directory, rather than PyInstaller's single-file form, because the single file's self-extracting shape is what antivirus heuristics on Windows most often flag. tauri.conf.json names the folder under `bundle.resources` in the directory form, `{"../engine/dist/ftorrent-engine": "ftorrent-engine"}`, and the form matters: the glob form, `.../**/*`, flattens every file into one directory and loses `_internal`, which we found out by building it. Tauri copies the folder into the bundle and, during development, next to the debug binary.

`src-tauri/src/engine.rs` is the whole of the app's side: it starts the process from `setup`, sends it an `init` line carrying the app's version from tauri.conf.json, keeps the `ready` line it answers with, holds the last hundred lines of its stderr, and stops it from the `ExitRequested` and `Exit` run events. `engine_status` is the one command the page has, wrapped by `src/engine.js`, and the main page asks once a second and prints the answer as a line. No shell plugin is registered, on purpose: the only code in the app that can start a process is that module. `engine.py` also holds, in one table, the centralized servers the engine will reach on its own, the STUN server, the DHT bootstrap nodes, and the trackers added to torrents ftorrent creates, and reports them in its `ready` line; the Names and Numbers document on [docs.ftorrent.com](https://docs.ftorrent.com/) is the public record of every entry, and of the names the client sends.
...
Wrapping the router outlet in `<KeepAlive>` would have kept the box filled too, and it is the more common reflex. It preserves the component instead of moving the state out of it, so the value stays welded to the one component that owns it, and a second component wanting the same fact still can't reach it. The reply from the Rust `greet` command is deliberately left as component state and still clears on navigation — the two behaviors sitting side by side in one small page are easier to see than to describe.

`<KeepAlive>` is the reflex, though, and not the honest competitor. That one is a module-scope ref: an ES module is a singleton, so a ref exported from a plain file already outlives every component and is importable anywhere — a store in all but name, at no dependency cost. What Pinia adds over that baseline is narrower than its reputation. `acceptHMRUpdate`, so editing a store during development doesn't wipe what it holds. `$patch` and `$subscribe`, for applying a set of changes as one unit and for watching what changed. The devtools timeline, which for the reason at the end of this section we don't collect. And a convention any Vue developer recognizes on sight — which, in a workspace meant to be read and forked, is most of the weight. We would rather spend a small dependency on a shape a reader already knows than grow a private one.

Stores are written in the setup form, `defineStore` with a function that returns what the store holds, which reads like the `<script setup>` components around them.
...
The engine was frozen and run on macOS in September 2026: the freeze takes under five seconds and produces a 45 MB folder; run by hand, from the freeze and again from inside the built bundle, it answers init with libtorrent 2.1.1.0, WebTorrent on, Python 3.13.15, frozen; the built app starts it within a second of launch as its own child process; and quitting the app, whether killed outright or asked through its own quit event, leaves no engine process behind. The engine line on the main page, the one place the app's own exchange with the engine is visible, is checked by a person looking at the window rather than by anything we measured. The Linux freeze was exercised the same day in Docker containers for both architectures and checked on a bare Debian 12, and the next day the same pipeline made the `.rpm` and the Flatpak too, the rpm's engine checked on a bare Fedora 40; the Linux guide in `linux/README.md` records what each check found.

The Windows freeze followed on 2026-Sep-22, on the same Windows 10 22H2 machine. uv installed its own Python beside a Python 2.7 already on the path and ignored it, as `python-preference` intends; the freeze took about ten seconds and produced a 33 MB folder; and run by hand, the frozen engine printed a ready line byte for byte identical to the Mac's. `cargo check` passed and copied the folder beside the debug binary. In development mode and again installed, the app started the engine as its own child with no console window, the engine line on the main page read `engine: libtorrent 2.1.1.0, WebTorrent on, Python 3.13.15` followed by the engine's pid, and closing the window ended both processes. `pnpm installer` took about seventy seconds and produced a 12 MB NSIS installer with the engine folder inside; installed, the app and engine come to 42 MB under `%LOCALAPPDATA%\ftorrent`, and the uninstaller removed that folder, the engine with it, along with the uninstall entry and the Start menu shortcut. Two things the uninstaller leaves by default, because Tauri's installer script removes them only when the user ticks its delete-app-data box: the registry key `HKCU\Software\ftorrent\ftorrent`, holding the install path and the installer language, and the WebView2 data folder under `%LOCALAPPDATA%\com.ftorrent.ftorrent`. Windows Defender, with real-time protection on, raised nothing at any step. SmartScreen is a separate check, and it did not appear, for a reason that says nothing about a real download: SmartScreen inspects only a file carrying the mark of the web, the `Zone.Identifier` stream a browser writes on download, and an installer built on the machine has none. A copy stamped with that stream by hand showed the blue "Windows protected your PC" screen, which "More info" and then "Run anyway" get past in two clicks; that is what a person downloading the unsigned installer from ftorrent.com will see until it is signed, and it is a reputation check on the file, not a finding inside it.

The first upload was on 2026-Sep-22. The Mac sent the dmg and the four Linux packages, the Windows box sent the exe, and all six, downloaded back from ftorrent.com, hashed to the sidecars committed in the repository.
