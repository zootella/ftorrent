...
`icons` is ahead of its artwork. It expects three SVG sources under `src-tauri/icons/`: `app-icon.svg` for every platform, `app-icon-mac.svg` inset for the Dock, and `app-icon-tile.svg` for the Windows Start menu tile. Until they exist it stops at the first with a file-not-found error, which is benign. The arrangement is written down now so the artwork drops into place when it arrives.
