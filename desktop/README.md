...
**Bundle targets.** The scaffold ships `"targets": "all"`, which builds everything each platform can build — on Windows an MSI beside the NSIS installer, on Linux an AppImage beside the Debian package. ftorrent distributes six packages, and the four that Tauri makes on this machine or the Windows one are named instead:
