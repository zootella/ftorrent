...
_Five guides cover this deployment: [dockerizing Aquatic and configuring the Linux server](../README.md), the [dashboard back end](../gauge/README.md), the [circuit breaker](../breaker/README.md), the **dashboard front end** (this guide), and the [DHT bootstrap node](../dht/README.md)._
...
The trick is a *noise function* — a stateless hash `f(i)` that maps each integer to a value that *looks* random but is fully determined by the input. Same i, same output, always. The one we use is [Squirrel3](https://www.youtube.com/watch?v=LWFzPP8ZbdU), introduced by Squirrel Eiserloh of SMU Guildhall in his 2017 GDC talk *Math for Game Programmers: Noise-Based RNG*. Three multiply-by-large-prime steps and three xor-shift "avalanche" steps in sequence: each one churns the bits of the previous one, and by the end any change to the input — even flipping a single bit — has propagated unrecognizably across the whole 32-bit output. Statistically uniform, fast, no internal state. Feed it today's UTC day index, take the result modulo the quote list length, render that quote. Tomorrow's index differs by one, but the avalanche turns that one-bit change into a totally different output, landing on a slot nowhere near today's.

The thing this buys us: no database, no cache, no last-quote-served file, no server-side anything. Every browser independently runs the same handful of arithmetic operations on the same input and reaches the same conclusion. The "shared state" everyone agrees on is the date itself.
