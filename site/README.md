...
This deployment shape isn't particular to Nuxt: the equivalent in [Next.js](https://nextjs.org/) is **static export** (`output: 'export'`), which writes a static `out/` directory with no Node server at runtime. Next prerenders each route's content into its HTML where our `ssr: false` build ships a blank, client-rendered shell, and a server hands the two out differently: a Next export writes a real HTML file per route, which the server returns straight from disk like a multi-page site, while this SPA writes one shell and routes every path back to it through `try_files`. What they share is narrower: both are static files that need no runtime to serve. In neither case does "static" mean serverless: a bundled SPA still needs a static HTTP server and won't open over `file://`, because its shell loads chunks from absolute, `/`-rooted paths and ES-module scripts require an `http(s)` origin (`pnpm preview` serves it locally). The only thing that needs no server at all is a single self-contained HTML file — which is what this page was before it became a bundle.
...
Two cache rules go alongside the fallback:
...
Below are these three pieces — the SPA fallback and the two cache rules — for Caddy and for nginx.
...
## The bookmark fragment

There's a third part of a URL that neither the file lookup nor the SPA fallback ever touches: the **fragment**, the part after `#`. Most readers know it by sight — the `#History` at the end of a Wikipedia link is a fragment, and clicking it scrolls you to that article's _History_ section. That's what fragments were made for: pointing at a place *within* a document, not at a different document. The browser fetches the page, then uses the fragment to find the spot — *which* document is the server's question, *where in* it is the browser's; named anchors have worked this way since the early web and still do.

In a client-rendered SPA that jump takes a little extra work, since the section a fragment points at isn't in the HTML the server returns — it only exists once the JavaScript renders the page. The browser's native scroll-to-anchor runs against the empty shell, finds nothing, and gives up; Vue Router fills the gap, re-running the scroll after the route's components mount (its `scrollBehavior`, which Nuxt wires up by default). Same-page anchor clicks still scroll natively, since the target is already in the DOM; cross-route ones ride the client-side navigation and scroll once the new page has rendered. The one case to watch is content that appears only after async data resolves — the scroll can fire before the element exists, which is what `scrollBehavior`'s `nextTick` and promise escape hatches are for.

### Privacy, as a side effect

Because that job always belonged to the browser, the web was built so the fragment is never sent to the server at all. (We'll watch it not arrive when we trace a request, below.) It isn't secret — it's right there in the URL bar and browser history, and anyone you share the link with sees it, the same as any link — but the **web server, as a piece of infrastructure, never receives it.** That's a small, durable idea: a feature whose input rides in the fragment can run entirely in the visitor's browser, with no back end at all.

### Paths, not the hash

There are two ways to route a single-page app, and the choice decides whether the fragment is free for any of this. We route through the **path** — `/about`, by the browser's History API — which is exactly why the server needs the `try_files` fallback above. The other common style is **hash routing**: `/#/about`, where the route itself lives in the fragment and the server only ever sees `/`. Hash routing needs no server config at all, which is why you find it wherever the server can't be touched — but it spends the fragment on the route. We paid the one line of `try_files` instead and bought the fragment back: with the route in the path, the `#` stays free to carry a payload, and the magnet below has somewhere to ride. Route through the hash and there'd be nowhere to put it — the route and the payload would be fighting over the same space.

### The magnet introducer

That's the foundation the magnet introducer is built on. A magnet link carried in the fragment (`/#/<magnet>`) is handed to the *page*, not the server — so the page can be a friendlier landing than a bare `magnet:` link, and do several things that raw link can't:
...
We have the three parts now — a static bundle (the client), a web server whose only rule is "real file, or else the shell," and a fragment the browser keeps to itself. Follow one request and watch them work together. Three kinds of link land on this one page, and the server treats each differently:
...
Say someone posts `https://ftorrent.com/#/some-magnet` somewhere public, and a visitor clicks it:
...
4. **The encrypted request goes out:** `GET / HTTP/2`. The path and any query string *are* sent to the server — encrypted on the wire, then decrypted and read by the server, which is how it knows which page you asked for. The fragment is **not in the request at all.** So "is the fragment encrypted?" doesn't quite apply: it isn't sent in the clear, it isn't sent encrypted, it isn't sent.
5. **The server resolves the path with `try_files`.** `/` is not a literal file, so it hands back the `index.html` shell — a plain `200`. (Had the path been `/logo.png`, a real file, it would have gone straight from disk; had it been `/about`, also no file, the shell again.) The response carries no idea of "which route," and nothing of the fragment the server never saw.
6. **The bundle boots and the browser takes over.** The entry chunk creates the Vue app and Vue Router, reads `window.location.pathname` to pick the route and render it, and reads `location.hash` to find the magnet. From here the page can inspect that magnet — all without the server ever having been told what it was.

After that first load the server mostly goes quiet. Navigation stays in the browser: clicking a `<NuxtLink>` has Vue Router intercept the click, push the new path with `history.pushState`, and re-render in place — no HTTP request at all. The back button fires `popstate`, the router reads the now-current path, and re-renders. The server is pulled back in only on a hard refresh, a URL pasted into a fresh tab, an external link in, or the fetch of a lazy route chunk — each of which is just another `try_files` lookup landing on the shell or a real file.

So the whole picture, a line each: **the path picks which document** — the server's call, answered from disk or the shell. **The route, and the scroll-to spot, are the browser's** — decided in the bundle after it boots. **And the fragment is the browser's alone** — never sent, which is exactly why the page can do real work on a magnet the server was never given.
