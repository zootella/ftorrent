...
Two settings in `tauri.conf.json` are written down rather than left to Tauri's defaults. First, the NSIS installer's `installMode` is pinned to `"currentUser"`, which installs into the user's own directory with no elevation and no wizard page asking whether to install for one user or for the whole machine. Tauri 2.11 already defaults to that mode, so the line changes no behavior today — it is there because the per-user install is load-bearing for everything else in this document, from where the binary lives to why no UAC prompt appears, and a default is not a promise. Second, the bundle targets are narrowed to the four packages ftorrent ships: Tauri's default of `"all"` builds everything each platform can produce, which adds an `.AppImage` on Linux and an `.msi` beside the NSIS installer on Windows. Both are one-line changes in the config file. Everything else in the distribution and filesystem layout follows Tauri's out-of-box defaults.
...
Uninstalling is the counterpart to the install flow in §1–§2, and ftorrent follows each platform's own convention rather than inventing one — no "are you sure you want to leave?" guilt screen, no self-deleting trick, the user stays in charge. On Windows that is the NSIS uninstaller (registered so it appears in Add or Remove Programs); on macOS it is dragging `ftorrent.app` to the Trash, the gesture every Mac user already knows; on Linux it is `apt remove` / `dpkg -r`.
...
**ftorrent warns before the OS interrupts.** Several of these actions hand off to the operating system's own confirmation — Windows elevates to add a firewall rule, macOS shows a dialog when an app sets a default handler or first touches a protected folder, macOS asks the user to approve a login item in System Settings. When ftorrent is about to trigger one of these, it says so first in its own UI, so the OS prompt is expected rather than a surprise. The OS dialog is the right place for the final yes — ftorrent asks for it, it does not try to route around it.
