...
description: A survey of the obstacles, protocols, libraries, and public services that let two ordinary computers on the internet reach each other directly.
---

# How Can Two Peers Connect?

The internet was built so that any two computers could reach each other directly — that is what an IP address is for. Today most cannot.

The reason isn't a decision anyone made. Direct reachability eroded as the by-product of a handful of ordinary choices, none of them aimed at the result, until the network had settled into the client-server shape it has today. But direct connection never stopped being needed, and so a second layer of protocols and techniques grew back over the first to recover it, connecting two computers straight through the obstacles that now sit between them.

This essay surveys both halves of that story: what closed the direct path, and what reopened it. It covers the protocols, the libraries that implement them, the public services anyone can call, and the choices a few real projects made when they needed two peers to meet. It is mostly about other people's work, and it describes the state of things in 2026.

## Why it matters

Somewhere in the last three decades the internet sorted its computers into two classes.
...
It isn't the shape the network was built in. The original design put every computer on equal footing: any host able to address any other, as ready to serve as to consume. What replaced it is an internet where the ability to be *reached* — to host, to publish, to accept a connection from a stranger — became a paid product, and where two ordinary people's computers can no longer talk to each other without a company in between.
...
Three separate things stand between two ordinary computers. They're often blurred together, and they have different causes and different fixes.
...
IPv6 ends the shortage outright. The space is large enough to give every device on Earth a globally routable address with room to spare. On a working IPv6 network there is no translation at all. Your computer's address is its real address, the one the rest of the internet would use, and it knows it without asking anyone. That dissolves half the problem this essay is about. The whole business of discovering *what your address is* goes away, and only *can a packet reach it* remains.
...
**Hole punching** is the technique that gets through. When a device inside sends a packet out, the router opens a temporary mapping so the reply can come back. If two peers each send outward at roughly the same moment, each one's outgoing packet creates the opening that lets the other's arrive. For IPv4 this does two jobs at once — it establishes the address translation *and* it satisfies the firewall. For IPv6 only the second job exists.

**Mapping behavior** decides whether the punch lands:

- **Endpoint-independent mapping** means the router assigns the same public port to your computer regardless of which destination you're sending to. A port you learned by asking one server is the port any peer can use. Hole punching works.
