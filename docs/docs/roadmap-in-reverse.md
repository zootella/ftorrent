...
The server runs [coturn](https://github.com/coturn/coturn), the reference implementation, on the standard port 3478. Our short configuration file mostly turns things off, with `stun-only` refusing TURN relaying and a handful of `no-` flags closing listeners we dont need. _stun.ftorrent.com_ joins our other public services as a small contribution to the decentralized web, and using it is as simple as adding `stun.ftorrent.com` to your list of STUN servers.

The STUN protocol is trivial: one request, one reply, with no server state needed. That single stateless exchange is so cheap to answer that three major tech corporations give it away freely — Google's `stun.l.google.com:19302`, Cloudflare's `stun.cloudflare.com:3478`, and Twilio's `global.stun.twilio.com:3478`, the servers nearly every WebRTC app already lists, together answering billions of requests a month. Note none of the three are really offered as a public STUN service in their own right. Google's has no documentation or terms at all — the single most-queried STUN server on the internet, sitting on a nonstandard port — while Cloudflare's and Twilio's surface mostly inside the docs for their paid TURN products. Also note an important privacy detail: while a STUN server is stateless, the company running it may already know a great deal about you at the IP address your app contacted their STUN server from, and that, not the protocol, is what really differs from one to the next.
