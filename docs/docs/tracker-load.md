...
Measured 2026-Jun-4, [open.ftorrent.com](https://open.ftorrent.com/) served **77 million** requests, moved **10.1 Mbit/s** inbound and **18.5 Mbit/s** outbound, ran at a 1-minute load average of **1.50** across its 28 hardware threads (~95% idle), held its entire working set in **0.96 GB** of RAM, kept a connection-tracking table peaking at **92,950** entries, dropped zero packets, and did all that drawing only **40 watts** at the wall. What follows is where each number comes from.
